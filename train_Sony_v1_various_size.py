# this code will be ran in tf2

from __future__ import division
import os, time, sys
import tensorflow as tf
import numpy as np
import rawpy, random, glob, PIL
tf.compat.v1.disable_eager_execution()

# train_size_scenes = [5, 10, 15, 20, 25]
...
result_dir = './results_Sony_on_train_' + str(size) + '/' 

# get train IDs
train_fns = glob.glob(gt_dir + '0*.ARW')
train_ids = [int(os.path.basename(train_fn)[0:5]) for train_fn in train_fns]

train_ids = random.sample(train_ids, size)

print("train set size= " + str(len(train_ids)) + "\n")
...
for epoch in range(lastepoch, 4001):
    ...
    for ind in np.random.permutation(len(train_ids)):
        ...
        if epoch % save_freq == 0:
            ...
            PIL.Image.fromarray((temp * 255).astype(np.uint8)).save(result_dir + '%04d/%05d_00_train_%d.jpg' % (epoch, train_id, ratio))
